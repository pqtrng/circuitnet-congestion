...
The rules are evaluated exactly as the training loop applies them: strict
improvement, so a tie keeps the earlier epoch. That correspondence is the point
of the probe. If it selected a different epoch than the loop did, the numbers
reported here would describe no checkpoint that exists on disk.

...
_Generated 2026-07-31 by `make report`. Every number below is produced from the
pipeline manifests, not written by hand._

_The pipeline runs as `make acquire` → `make bronze` → `make silver` → `make gold`
→ `make report`. The raw download is roughly 24 GB and the intermediate layers
need about 40 GB of free disk, so rebuilding from scratch is not a quick check.
The Silver manifest and Gold summary are committed as evidence so the figures
below can be audited without rebuilding; the layer data itself is not committed
and must be regenerated by the pipeline._

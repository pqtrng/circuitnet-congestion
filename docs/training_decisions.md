...
Patch coverage ranges from under two percent to a full window, because edge
patches are zero-padded to a fixed size. Reducing the loss as a mean of per-patch
means would let a pixel in a small edge fragment count up to sixty-four times a
pixel in a full patch. The reduction is therefore a single global masked mean
...
value 0.07 sits below any usable threshold. The model then predicts a smooth
...
Per-image time is flat across the entire batch range this accelerator can hold,
so no batch size in that range is faster than another. The choice is made on
memory headroom instead, and for a specific reason: on this platform, exceeding
device memory does not raise. The driver falls back to host memory and the run
becomes several times slower with nothing in the logs to say so.

That failure was observed directly before autotuning was enabled, and it does not
reproduce with autotuning on, which selects a more frugal convolution algorithm
under pressure. Both measurements describe their own configuration correctly. The
conservative batch size is kept because the failure it avoids is silent, not
because the faster option is slower.
...
do not reproduce. Both were true when they were made: the output head was not yet
zero-initialised, predictions ran two orders of magnitude above any target, and
the resulting errors genuinely did overflow. Fixing the initialisation removed
the numerical problem and left the conclusion without a justification.

The justification that survives is throughput. This accelerator has no dedicated
matrix units, so a narrower format buys no arithmetic and the cast is pure
overhead. Bfloat16 is measured alongside as a control rather than a candidate: it
carries single precision's exponent range with fewer mantissa bits, so a failure
of range would separate the two formats. It does not.
...
This section exists in its current form because the probe behind it was wrong
three times.
...
one already written into a commit message.
...
it sweeps. That costs roughly three and a half times the runtime, which is the
...
initialisation rather than platform noise — and the ranking of the candidate
rates reverses.
...
The baseline was trained three times. Each retraining was caused by a measurement
that contradicted a standing assumption: early stopping selecting on noise, then
